...
from .cluster import list_cluster_nodes, list_model_templates
from .jobs import (
...
@app.get("/datasets")
def datasets_list():
    """List datasets available to the user — Hub, local cache, and NAS.

    NAS entries also carry the absolute shared ``dataset_root``.
    """
...
def cluster_status():
    """Return Slurm state plus out-of-band GPU process probes."""
    return list_cluster_nodes()


@app.get("/cluster/templates")
def cluster_templates():
    """Return the administrator-registered model templates."""
    try:
        return {"templates": list_model_templates()}
    except ValueError as exc:
...
def resume_job(job_id: str, body: ResumeJobRequest | None = None):
    try:
        return job_registry.resume(job_id, body.node if body else None)
    except JobNotFoundError as exc:
        raise HTTPException(status_code=404, detail=f"Job {job_id!r} not found") from exc
    except JobAlreadyRunningError as exc:
        raise HTTPException(status_code=409, detail=f"Job {job_id!r} is already running") from exc
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc

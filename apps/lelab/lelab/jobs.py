...
class JobTarget(BaseModel):
    ...
    ``slurm`` accepts an optional node name; omitting it (or using ``auto``)
    selects the currently idle GPU node with the most free memory.
    """

    runner: Literal["local", "hf_cloud", "slurm"] = "local"
...
class JobRecord(BaseModel):
    ...
    runner: Literal["local", "hf_cloud", "slurm", "imported"] = "local"
    ...
    # Slurm identifiers (slurm runner only).
...
def _generate_job_id(policy_type: str, dataset_repo_id: str) -> str:
    ...
    # Five users can submit the same template/dataset within one second.
    # Microseconds keep the filesystem key collision-free without a separate
    # database sequence.
...
class JobRegistry:
    ...
    HF jobs reattach to the remote API, and Slurm jobs reattach to the
    controller and shared log file.
    """
    ...
    def start(self, config: TrainingRequest, target: JobTarget | None = None) -> JobRecord:
        ...
        from .runners.slurm import SlurmJobRunner
        ...
        if target.runner == "slurm":
            from .cluster import get_model_template

            template = get_model_template(config.policy_type)
            if template.policy_type != config.policy_type:
                raise ValueError(
                    f"Template {template.id!r} runs policy {template.policy_type!r}, "
                    f"not {config.policy_type!r}"
                )
        ...
        with self._lock:
            ...
            if target.runner == "local":
                ...
                reserved_nodes = {
                    ...
                    if item.id != job_id
                    and item.runner == "slurm"
                    and item.state == "running"
                    and item.node_name
                }
                runner = SlurmJobRunner(record.metrics, log_path, target.node, reserved_nodes)
            ...
            elif target.runner == "slurm":
                record.slurm_job_id = runner.slurm_job_id()
                record.node_name = runner.node_name()
    ...
    def resume(self, job_id: str, node: str | None = None) -> JobRecord:
        """Resume a failed/interrupted Slurm job from its newest checkpoint."""
        ...
        with self._lock:
            ...
            self._persist(record, force=True)
            try:
                runner.start(job_id, record.config, record.output_dir)
            except Exception as exc:
                self._runners.pop(job_id, None)
                record.state = "failed"
                record.ended_at = time.time()
                record.error_message = f"Failed to resume runner: {exc}"
                self._persist(record, force=True)
                raise
            record.slurm_job_id = runner.slurm_job_id()
            record.node_name = runner.node_name()
    ...
    def _checkpoints_for(self, record: JobRecord) -> builtins.list[JobCheckpoint]:
        ...
        if record.runner in {"local", "slurm"}:
        ...
        for job_dir in self._output_root.glob("*/"):
            ...
            if record.state == "running":
                if record.runner == "local":
                    ...
                    self._runners[record.id] = runner
                elif record.runner == "slurm" and record.slurm_job_id:
                    logger.info(
                        "Re-attaching to Slurm job %s (slurm_job_id=%s)",
                        record.id,
                        record.slurm_job_id,
                    )
                    from .runners.slurm import SlurmJobRunner
                    ...
                    runner.reattach(record.slurm_job_id, record.node_name, record.output_dir)
                    self._runners[record.id] = runner
                else:

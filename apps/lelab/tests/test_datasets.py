...
def test_list_nas_datasets_exposes_shared_root(tmp_path, monkeypatch) -> None:
    from lelab.datasets import list_nas_datasets

    nas = tmp_path / "nas-datasets"
    _make_dataset(nas, "team/pick-cube")
    monkeypatch.setenv("LELAB_NAS_DATASETS_ROOT", str(nas))

    result = list_nas_datasets()

    assert len(result) == 1
    assert result[0]["repo_id"] == "team/pick-cube"
    assert result[0]["dataset_root"] == str((nas / "team/pick-cube").resolve())

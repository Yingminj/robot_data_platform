...
function configToRequest(c: TrainingConfig): TrainingRequest {
  ...
  return {
    ...
    dataset_image_transforms: c.dataset_image_transforms,
...
const ConfigurationMode: React.FC = () => {
  ...
  const [trainingConfig, setTrainingConfig] = useState<TrainingConfig>({
    target: { runner: "slurm", node: "auto" },
    dataset_repo_id: prefilledDatasetRepoId,
    dataset_image_transforms: false,
    policy_type: "act",
    steps: 10000,
    batch_size: 8,
    seed: 1000,
  ...
  useEffect(() => {
    ...
    listDatasets(baseUrl, fetchWithHeaders)
      .then((items) => {
        setDatasets(items);
        if (prefilledDatasetRepoId) {
          const selected = items.find((item) => item.repo_id === prefilledDatasetRepoId);
          setTrainingConfig((previous) => ({
            ...previous,
  ...
  }, [baseUrl, fetchWithHeaders, prefilledDatasetRepoId]);
  ...
  useEffect(() => {
    let cancelled = false;
    const refresh = () => {
      getClusterStatus(baseUrl, fetchWithHeaders)
        .then((status) => {
          if (!cancelled) {
            setClusterEnabled(status.enabled);
            setClusterNodes(status.nodes);
          }
        })
        .catch(() => {
          if (!cancelled) {
  ...
  useEffect(() => {
    listModelTemplates(baseUrl, fetchWithHeaders)
      .then((items) => {
        setTemplates(items);
        setTrainingConfig((previous) =>
          items.length > 0 && !items.some((item) => item.id === previous.policy_type)
            ? { ...previous, policy_type: items[0].id }
            : previous,
        );
      })
      .catch(() => setTemplates([]));
  ...
  if (trainingConfig.target.runner === "local" && trainingExtraAvailable === null) {
    return (
  ...
  if (trainingConfig.target.runner === "local" && trainingExtraAvailable === false) {
    return (
      <div className="min-h-screen bg-slate-900 text-white p-4">
        <div className="max-w-7xl mx-auto">
          <TrainingHeader />
  ...
  const selectedClusterNode =
    trainingConfig.target.runner === "slurm" &&
    trainingConfig.target.node &&
    trainingConfig.target.node !== "auto"
      ? clusterNodes.find((node) => node.name === trainingConfig.target.node)
      : undefined;
  const clusterBlocked =
    trainingConfig.target.runner === "slurm" &&
    (!clusterEnabled ||
      (selectedClusterNode
        ? !selectedClusterNode.eligible
        : !clusterNodes.some((node) => node.eligible)));
  const startDisabled =
    isStarting ||
  ...
  const startTooltip = localBlocked
    ...
    : clusterBlocked
    ? "No eligible cluster GPU is currently idle"
    : targetRequiresAuth && !authenticated
  ...
  const handleResume = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: next.node_name ? `Running on ${next.node_name}` : undefined,
      });
    } catch (e) {
      toast({
        title: "Resume failed",
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                {job.runner === "hf_cloud" ? (
                  ...
                  <span className="text-xs px-2 py-0.5 rounded bg-blue-900/40 text-blue-200 border border-blue-700">
                    Slurm · {job.node_name ?? "queued"}
                    {job.slurm_job_id ? ` · #${job.slurm_job_id}` : ""}
                  </span>
                ) : (
          ...
          {isRunning ? (
            ...
            <div className="flex items-center gap-2">
              {job.runner === "slurm" && job.checkpoint_count > 0 && (
                <Button onClick={handleResume} className="bg-blue-500 hover:bg-blue-600 text-white">
                  <Play className="w-4 h-4 mr-2" /> Resume from checkpoint
                </Button>
              )}

...
import { ModelTemplate } from '@/lib/jobsApi';
        ...
        <div>
          ...
          <div className="mt-1">
            <DatasetCombobox
              ...
              onChange={(repoId) => {
                if (!repoId) return;
                updateConfig('dataset_repo_id', repoId);
                const selected = datasets.find((item) => item.repo_id === repoId);
                updateConfig('dataset_root', selected?.dataset_root);
              }}
            />
          </div>
          <p className="text-xs text-slate-500 mt-1">
            NAS datasets use their shared path directly; Hub datasets keep using their repository ID.
          </p>
        </div>

...
const JobsSection: React.FC = () => {
  ...
  const localJobs = useMemo(
    () => filteredJobs.filter((j) => j.runner === "local" || j.runner === "slurm"),
    [filteredJobs],
  );
    ...
    <section className="space-y-6">
      ...
      <Collapsible defaultOpen>
        <CollapsibleTrigger className="group flex items-center gap-1.5 text-sm font-semibold uppercase tracking-wide text-slate-400 hover:text-white transition-colors">
          ...
          Cluster and local jobs ({localActive.length})
        </CollapsibleTrigger>
        <CollapsibleContent className="pt-3">
          {localActive.length === 0 ? (
            <p className="text-sm text-slate-500">
              {query
                ? "No cluster or local jobs match your search."
                : "No active cluster jobs. Start one from the Training page."}
            </p>
          ) : (

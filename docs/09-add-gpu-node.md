...
首次部署不要看本文，看 [README](../README.md) 的完整顺序。
...
新节点如果是全新的仓库副本，先 `cp config/site.env.example config/site.env`，再核对 [README 中必须一致的字段清单](../README.md#0-准备统一配置)。`DATA_GID` 和 `TRAIN_UID` 尤其重要：数字不一致会导致作业在新节点上写不了 NAS。
...
在新节点上，与 [GPU 节点安装](03-gpu-node.md) 完全相同：
...
然后按 [Slurm 26.05.2 安装](Slurm-INSTALL.md) 安装与现有节点**完全相同**的包版本：
...
# "Could not load libtorchcodec"（见[排障 12b-2](08-troubleshooting.md)）。
sudo -u robot-train /opt/robot-platform/train-venv/bin/python -c \
...
| `GPU has a compute process outside or inside Slurm` | 该节点 GPU 上有 Slurm 外的 CUDA 进程，见[排障 10](08-troubleshooting.md) |

...
[English](02-management-node.md) | **简体中文**
...
向已经运行的集群加 GPU 节点不需要重做本文，见[向已有集群增加 GPU 节点](09-add-gpu-node.zh-CN.md)。
...
使用自建 DEB 后不要随意重新安装 Ubuntu 的 `slurm-wlm`，避免把 26.05.2 替换回旧版。详见 [Slurm 26.05.2 安装](Slurm-INSTALL.zh-CN.md)。
...
1. 所有主机安装 [Slurm 26.05.2](Slurm-INSTALL.zh-CN.md)；
2. 按 [Slurm 集群收尾](06-cluster-finalization.zh-CN.md)渲染配置、分发 Munge 密钥；
...
安装脚本不会覆盖已经存在的 `/etc/robot-platform/lelab.env`。SSH 探测配置必须按 [leLab 集群 Web](07-lelab-cluster-web.zh-CN.md)单独完成。

...
向已经运行的集群加节点时，除本文外还有一批集群级改动，见[向已有集群增加 GPU 节点](09-add-gpu-node.zh-CN.md)。
...
全集群必须安装完全相同的 Slurm 主版本和包构建。按 [Slurm 26.05.2 DEB 安装说明](Slurm-INSTALL.zh-CN.md)执行后检查：

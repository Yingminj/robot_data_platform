...
ensure_system_user "${INGEST_USER}" "${INGEST_UID}" "${DATA_GROUP}"
...
if [[ ! -d "${PLATFORM_ROOT}" ]]; then
  warn "${PLATFORM_ROOT} does not exist. Create the platform directory and ACLs in QNAP before starting services."
fi
...
log "next: create QNAP directories/ACLs, then run deploy/management/bootstrap.sh --apply"
